...
package com.hongenit.picseei18n.widgets.drawer;
...
/**
 * RightViewDragHelper is a utility class for writing custom ViewGroups. It offers a number
 * of useful operations and state tracking for allowing a user to drag and reposition
 * views within their parent ViewGroup.
 */
public class ViewDragHelper {
    private static final String TAG = "RightViewDragHelper";
    ...
    private static final int EDGE_SIZE = 250; // dp
    ...
    /**
     * A Callback is used as a communication channel with the RightViewDragHelper back to the
     * parent view using it. <code>on*</code>methods are invoked on siginficant events and several
     * accessor methods are expected to provide the RightViewDragHelper with more information
    ...
    public abstract static class Callback {
         ...
         * one of these methods, the RightViewDragHelper will enter {@link #STATE_SETTLING}
         * and the view capture will not fully end until it comes to a complete stop.
         * If neither of these methods is invoked before <code>onViewReleased</code> returns,
         * the view will stop in place and the RightViewDragHelper will return to
         * {@link #STATE_IDLE}.</p>
         *
         ...
         *
         * <p>RightViewDragHelper may call this method multiple times for the same view even if
    ...
    /**
     * Factory method to create a new RightViewDragHelper.
     *
     * @param forParent Parent view to monitor
     * @param cb Callback to provide information and receive events
     * @return a new RightViewDragHelper instance
     */
    ...
    /**
     * Apps should use RightViewDragHelper.create() to get a new instance.
...
//        dm.widthPixels
    ...
    public void captureChildView(View childView, int activePointerId) {
        if (childView.getParent() != mParentView) {
            throw new IllegalArgumentException("captureChildView: parameter must be a descendant "
                    + "of the RightViewDragHelper's tracked parent view (" + mParentView + ")");
        }
     ...
     * of the RightViewDragHelper's knowledge).
     *
    ...
    public boolean shouldInterceptTouchEvent(MotionEvent ev) {
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_POINTER_DOWN: {
                ...
                // A RightViewDragHelper can only manipulate one view at a time.
                if (mDragState == STATE_IDLE) {
                    final int edgesTouched = mInitialEdgesTouched[pointerId];
                    if ((edgesTouched & mTrackingEdges) != 0) {
                        mCallback.onEdgeTouched(edgesTouched & mTrackingEdges, pointerId);
    ...
    private boolean isValidPointerForActionMove(int pointerId) {
        if (!isPointerDown(pointerId)) {
            Log.e(TAG, "Ignoring pointerId=" + pointerId + " because ACTION_DOWN was not received "
                    ...
                    + " RightViewDragHelper did not receive all the events in the event stream.");
            return false;
        }
        return true;
    }
